...
public class GameController
{
    ...
    public static bool PlaceFigure(Figure figure, int baseRow, int baseCol, Panel[,] cells, int gridSize)
    {
    ...
    public void CheckAndClearLines(Panel[,] cells, int gridSize)
    {
        var linesCleared = 0;
    ...
    private static void ClearLine(int startX, int startY, int stepX, int stepY, Panel[,] cells, int gridSize)
    {
        for (var i = 0; i < gridSize; i++)
        {
    ...
    public static bool CanPlayerPlaceAnyFigure(Player player, Panel[,] cells, int gridSize) => 
        player.Figures.Any(figure => CanPlaceFigure(figure, cells, gridSize));

    private static bool CanPlaceFigure(Figure figure, Panel[,] cells, int gridSize)
    {
        for (var row = 0; row < gridSize; row++)
        for (var col = 0; col < gridSize; col++)
        {
            var canPlace = !(from pix in figure.Pixels let x = row + pix.X let y = col + pix.Y where x < 0 || x >= gridSize || y < 0 || y >= gridSize || cells[x, y].BackColor != Color.White select x).Any();
