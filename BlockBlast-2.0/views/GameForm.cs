...
namespace BlockBlast_2._0
{
    public partial class GameForm : Form
    {
        ...
        private void DrawPlayerFigures()
        {
            ...
            // Рисуем фигуры для каждого игрока
            for (int i = 0; i < controller.Players.Count; i++)
            {
        ...
        private void FieldPanel_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            {
                var color = controller.CurrentPlayer.Figures.FirstOrDefault()?.Pixels.FirstOrDefault()?.Color 
                    ?? Color.Blue.ToArgb();
                controller.CurrentPlayer.GenerateFigures(color);
                DrawPlayerFigures();
            }
            ...
            if (controller.CanPlayerPlaceAnyFigure(controller.CurrentPlayer, cells, GridSize)) return;
        ...
        private void EndGame()
        {
            ...
            var winnerText = controller.Players
                .GroupBy(p => p.Score)
                .Count() == 1 
                ? "Ничья!" 
